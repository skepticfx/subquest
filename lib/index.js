...
exports.getSubDomains = function (options, callback = () => {}) {
  ...
  let dictionaryFile = path.resolve(options.dictionary);
  // Allow previous behavior for `top_100` string input
  if (exports.getDictionaryNames().includes(`${options.dictionary}.txt`)) {
    dictionaryFile = path.join(dictionariesPath, `${options.dictionary}.txt`);
  }
  ...
  async.waterfall([
    ...
    (wildcard, cb) => {
      ...
      exports.getResolvers(options.dnsServer, servers => {
        ...
        let dictionary = fs.readFileSync(dictionaryFile).toString().trim().split(EOL);
